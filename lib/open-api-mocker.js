...
class OpenApiMocker {
	...
	async validate() {
		...
		try {
			...
			const { securitySchemes } = this.schema.components;
			const { security } = this.schema;
			const securityData = {};
			if(securitySchemes && Object.keys(securitySchemes).length > 0) {
				Object.entries(securitySchemes).forEach(securityItem => {
					const [securityKey, securityValue] = securityItem;
					if(securityValue.type !== 'oauth2' && securityValue.type !== 'openIdConnectUrl')
						securityData[securityKey] = securityValue;
				});
			}
	...
	async mock() {
		await this.options.server
			...
			.setSecurityConfig(this.api.securityConfig)
			.init();
